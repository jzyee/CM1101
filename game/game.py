...
def normalise_input(user_input):
    ...
    newInput = remove_punct(user_input)
    newInput = remove_spaces(newInput)
...
def print_menu(exits):
    ...
    #for index,ch in exits.items():

    for direction in exits:
        extLds = exit_leads_to(exits,direction)
        print_menu_line(direction,extLds)
...
def is_valid_exit(exits, user_input):
    ...
    for movements in exits:
        if(movements == user_input ):
            checkExit = True
...
def menu(exits):
    ...
    while True:
        ...
        if (is_valid_exit(exits,userInput)): 
            return userInput
